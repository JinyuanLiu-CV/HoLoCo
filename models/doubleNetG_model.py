...
class SingleModel(BaseModel):
    ...
    def backward_G(self):
        ...
        if(self.opt.fullinput == 0):
            ...
            if(self.opt.hasglobal == 1 and self.opt.patchD_3 == 0):
                self.loss_G = mw * self.mse_loss + gw*self.loss_G_A + vw * \
    ...
    def forward(self):
        ...
        if self.opt.skip == 1:
            self.fake_C1, self.latent_fake_A1 = self.netG_A.forward(
                self.fake_B, self.fake_B_gray.cuda())
            self.fake_C2, self.latent_fake_A2 = self.netG_B.forward(
                self.fake_B, self.fake_B_gray.cuda())
