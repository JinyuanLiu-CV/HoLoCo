...
class SingleModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        self.netG_A.cuda()
        print(sum(param.numel()) for param in self.netG_A.parameters())
    ...
    def forward(self):
        ...
        if self.opt.skip == 1:
           # self.fake_C, self.latent_fake_A = self.netG_A.forward(self.fake_B,self.fake_B_gray)
            self.fake_C1, self.latent_fake_A1 = self.netG_A.forward(self.fake_B,self.fake_B_gray.cuda())
            self.fake_C2, self.latent_fake_A2 = self.netG_B.forward(self.fake_B,self.fake_B_gray.cuda())
            self.fake_C = (self.fake_C1 + self.fake_C2)/2
    ...
    def get_current_visuals(self):
        ...
        #fake_C1 = util.tensor2im(self.fake_C1.data)
        #fake_C2 = util.tensor2im(self.fake_C2.data)
